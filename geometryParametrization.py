...
class HEXTestrigDuctCurvedFinsGeometry:  # class definition containing the geometry for a duct in terms of bezier curves
    def __init__(self, baseGeometryDict):
        ...
        self.P_18_tangent = (self.P_1- self.P_8) / np.linalg.norm(self.P_1 - self.P_8)
        self.P_23_tangent = (self.P_2- self.P_3) / np.linalg.norm(self.P_2 - self.P_3)

        self.P_AE = self.P_8 + baseGeometryDict["lambda9"] * (self.P_1 + self.P_18_tangent * inlet_height - self.P_8)
        self.P_BF = self.P_3 + baseGeometryDict["lambda10"] * (self.P_2 + self.P_23_tangent * inlet_height - self.P_3)
